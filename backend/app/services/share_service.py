...
class ShareService:
    ...
    def get_record(self, record_id: int) -> dict[str, Any] | None:
        rows = self._fetch_all(
            ...
            LEFT JOIN tuning_part tp
                ON tsp.id_tuning_part = tp.id_tuning_part
            LEFT JOIN tuning_setup ts
                ON wr.id_tuning_setup = ts.id_tuning_setup
    ...
    def share_page(self, record_id: int) -> str:
        ...
        description = (
            ...
            or "HCR2 adventure world record"
        )

        safe_title = html.escape(title, quote=True)
        safe_description = html.escape(description, quote=True)
        safe_url = html.escape(
            f"/api/v1/share/records/{record_id}",
            quote=True,
        )
        safe_image = html.escape(
            f"/api/v1/share/records/{record_id}.png",
            quote=True,
        )
        target = html.escape(
            f"/records?map={map_name}&vehicle={vehicle_name}",
            quote=True,
        )
...
<p><a href="{target}">hcr2.xyz — {safe_title}</a></p>
    ...
    def og_image(self, record_id: int) -> bytes:
        ...
        if cache_dir is not None:
            try:
                ...
                target = cache_dir / self._cache_filename(
                    record_id,
                    row,
                )
    ...
    def _render_image(self, row: dict[str, Any]) -> bytes:
        ...
        font_hero = _load_font(bold_path, 86)
        font_h1 = _load_font(bold_path, 50)
        ...
        # Status badge stays aligned with the distance.
        self._draw_badge(
            draw,
            row,
            bold_path,
        ...
        if country_raw:
            player_line = f"{player_line} ({country_raw})"

        player_text = _fit_text_width(
            draw,
            player_line,
            font_body,
            700,
        ...
        parts = [
            p.strip()
            for p in str(row.get("tuning_parts") or "").split(",")
            if p.strip()
        ]

        metadata_parts: list[str] = []

        tuning_text = _truncate_parts(parts)

        if tuning_text:
            metadata_parts.append(tuning_text)

        echo_part = row.get("echoAffectedPart")
    ...
    def _gradient(
        ...
        for y in range(height):
            ...
            color = tuple(
                int(
                    top_channel
                    + (bottom_channel - top_channel) * ratio
                )
                for top_channel, bottom_channel in zip(
                    top,
                    bottom,
                    strict=True,
                )
